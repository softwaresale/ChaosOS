...
static int is_format_letter(char c) {
    return c == 'c' ||  c == 'd' || c == 'i' ||c == 'e' ||c == 'E' ||c == 'f' ||c == 'g' ||
	    c == 'G' ||c == 'o' ||c == 's' || c == 'u' || c == 'x' || c == 'X' || c == 'p' || c == 'n';
}

static void vsprintf_helper(char * str, void (*putchar)(const char), const char * format, uint32_t * pos, va_list arg) {
    char c;
    int sign, ival, sys;
    char buf[512];
    char width_str[10];
...
static void vsprintf(char * str, void (*putchar)(const char), const char * format, va_list arg) {
    uint32_t pos = 0;
